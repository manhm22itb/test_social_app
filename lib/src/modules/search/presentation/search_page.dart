  ...
  Widget _buildUsersList(SearchResult result) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (ctx, i) => UserItem(
        user: result.users[i], 
        onTap: () {
          Navigator.pushNamed(context, '/user/${result.users[i].username}');
        }
      ),
    );
  }
      ...
      child: Column(
        ...
        children: [
          if (result.users.isNotEmpty) ...[
            ...
            ...result.users.take(3).map((u) => UserItem(user: u, onTap: (){})), // Show tối đa 3 user
          ...
          if (result.posts.isNotEmpty) ...[
             ...
             // Hiển thị list posts (dùng ListView.shrinkWrap hoặc Column để cuộn chung)
             ListView.separated(
                shrinkWrap: true,
